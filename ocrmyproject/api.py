...
def ocr_directory(source_dir, destination_dir, language='fra', force_ocr=False, progress_callback=None):
    """
    ...
    # Collect all files and calculate total pages
    for root, _, files in os.walk(source_dir):
        for file in files:
            ...
            if file.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.tiff', '.tif')):
                ...
                if file.lower().endswith('.pdf'):
                    ...
                    total_pages += 1  # Images become 1-page PDFs
                files_to_process.append(full_path)

    # Count already processed pages
    ...
    for source_file_path in files_to_process:
        if source_file_path in processed_files and processed_files[source_file_path] == "completed":
            # Add pages from already processed file to the count
            if source_file_path.lower().endswith('.pdf'):
                try:
                    import fitz  # PyMuPDF
                    doc = fitz.open(source_file_path)
                    processed_pages_count += len(doc)
                    doc.close()
                except:
                    processed_pages_count += 1
            else:
                processed_pages_count += 1  # Images become 1-page PDFs
            processed_count += 1

    logging.info(f"Total files to process: {total_files}, Total pages: {total_pages}, Already processed: {processed_count} files ({processed_pages_count} pages)")
    ...
    for root, dirs, files in os.walk(source_dir):
        ...
        for file in files:
            ...
            if file.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.tiff', '.tif')):
                ...
                processed_pages_count += pages_in_file
                processed_count += 1

                progress_percentage = (processed_count / total_files) * 100 if total_files > 0 else 0
                pages_progress_percentage = (processed_pages_count / total_pages) * 100 if total_pages > 0 else 0

                logging.info(f"Progress: {processed_count}/{total_files} files ({progress_percentage:.2f}%), {processed_pages_count}/{total_pages} pages ({pages_progress_percentage:.2f}%)")

                # Call progress callback if provided (for GUI)
                if progress_callback:
                    progress_callback(processed_count, total_files, processed_pages_count, total_pages)
...
def process_file_with_ocr(file_path, output_path, language='fra', force_ocr=False):
    ...
    Explicit approach: Convert images to PDF first, then apply OCR.
    Uses OS-specific temporary directory for intermediate files.
    """
    # Standard library imports
    import shutil
    import tempfile
    ...
    logging.info(f"Processing file with OCR: {base_name}")

    try:
        if file_path.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.tif')):
            # For images: convert to temporary PDF first, then apply OCR
            ...
            with Image.open(file_path) as img:
                ...
                # Create temporary PDF from image with good resolution
                # Use OS-specific temporary directory
                with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False, dir=tempfile.gettempdir()) as temp_pdf:
                    temp_pdf_path = temp_pdf.name
                    # Save with good resolution for OCR
            ...
            try:
                ...
                ocrmypdf_options = {
                    ...
                    'force_ocr': force_ocr,  # Force l'OCR même si le PDF a déjà du texte
                    'optimize': 0,  # Optimise la taille du fichier
                    'progress_bar': False,  # Désactive la barre de progression interne
                    'skip_text': False,  # S'assure de traiter toutes les pages
                    'clean_final': True,  # Nettoie le PDF final
                    'skip-big': True,
                    'oversample': 300,  # Sur-échantillonne les images pour meilleure qualité OCR
                }
                ...
                logging.info(f"Image converted to PDF and processed with OCR: {output_path}")
                return True
            except Exception as ocr_error:
                logging.warning(f"OCR failed for image {base_name}, keeping original image as PDF: {ocr_error}")
                # If OCR fails, just copy the initial PDF (without OCR) to output location
                import shutil
            ...
            # For PDFs: apply OCR directly
            logging.info(f"Processing PDF: {base_name}")
            ...
            try:
                ...
                logging.warning(f"OCR failed for PDF {base_name}, keeping original PDF: {ocr_error}")
                # If OCR fails, copy the original PDF to output location
                import shutil
        ...
        # If everything fails, try to copy the file as-is
        try:
            if file_path.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.tif')):
                # Convert image to PDF without OCR as last resort
                with Image.open(file_path) as img:
                    rgb_image = img.convert('RGB') if img.mode in ('RGBA', 'LA', 'P', 'CMYK') else img
                    rgb_image.save(output_path, "PDF", resolution=150)
                logging.info(f"Image converted to PDF without OCR: {output_path}")
                return True
            elif file_path.lower().endswith('.pdf'):
                import shutil
                shutil.copy2(file_path, output_path)
                logging.info(f"Original PDF copied without OCR: {output_path}")
                return True
        except Exception as final_error:
            logging.error(f"Failed to process '{base_name}' and fallback also failed: {final_error}")
            return False
