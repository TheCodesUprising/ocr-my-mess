...
def test_cli_build_and_run():
    ...
    """
    executable_path = DIST_PATH / "ocr-my-mess-cli"

    # 1. Run the build script
    build_process = subprocess.run([sys.executable, str(BUILD_SCRIPT_PATH), "cli"], capture_output=True, text=True, check=False)
    ...
    assert "Usage: ocr-my-mess-cli" in run_process.stdout
...
def test_gui_build_and_run():
    ...
    """
    executable_path = DIST_PATH / "ocr-my-mess-gui"

    # 1. Run the build script
    build_process = subprocess.run([sys.executable, str(BUILD_SCRIPT_PATH), "gui"], capture_output=True, text=True, check=False)
    assert build_process.returncode == 0, f"Build script failed: {build_process.stderr}"
